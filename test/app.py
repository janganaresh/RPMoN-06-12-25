...
def generate_remedy_report():
    ...
    # ✅ Validation logic: only one mode of filtering should be used
    if (from_date and to_date and completed_date) or ((not from_date or not to_date) and not completed_date):
        return jsonify({"error": "Please either provide From + To Dates or a Completed Date — not both."}), 400
    ...
    try:
        query = """
            SELECT 
                r.`Remedy ID`,
                r.`Area ID`,
                a.`Assessment Case` AS `Assessed Case`,
                a.`Assessment Status` AS `Remedy Status`,
                r.`Pile No`,
                r.`Table ID`,
                DATE_FORMAT(r.`Task Date`, '%d %b %Y') AS `Task Date`,
                u.`User Name`
            FROM remedy r
            JOIN assessment a ON r.`Area ID` = a.`Area ID` 
        ...
        # ✅ Add condition for completed_date or from/to range
        if completed_date:
        ...
        cursor.execute(query + " ORDER BY r.`Remedy ID` ASC", params)
